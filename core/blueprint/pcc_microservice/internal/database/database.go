...
func (s *service) WriteTransaction(transaction Transaction) error {
	...
	query := `INSERT INTO transactions (transaction_id, status, timestamp, acquirer_id, issuer_id) 
	          VALUES ($1, $2, $3, $4, $5)`

	// Use the database connection to execute the query.
	_, err := s.db.Exec(query, transaction.TransactionId, transaction.Status, transaction.Timestamp, transaction.AcquirerId, transaction.IssuerId)
