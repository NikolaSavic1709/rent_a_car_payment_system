...
func (s *Server) RegisterRoutes() http.Handler {
	...
	authorized.Use(s.AuthMiddleware())
	{
		...
		// Rute za proizvode
		authorized.GET("/products", s.ListProducts)                       // Listanje proizvoda
		authorized.GET("/products/:id", s.GetProductByID)                 // Dohvatanje proizvoda po ID
		authorized.POST("/products", Authorize("admin"), s.CreateProduct) // Kreiranje proizvoda (samo admin)
		authorized.POST("/products/purchase", s.PurchaseProduct)          // Kupovina proizvoda

		// Rute za korisničke informacije
